...
public class StatePlaying extends DefaultState {
    ...
    PlayManuallyActivity controlManually = null;
    PlayAnimationActivity controlAnimation = null;
    // debug stuff
    ...
    public void setMazeConfiguration(Maze config) {
        mazeConfig = config;
        /*MazeFileWriter.store("input.xml", mazeConfig.getWidth(), mazeConfig.getHeight(),
        		2, 900, mazeConfig.getRootnode(), mazeConfig.getFloorplan(),
        		mazeConfig.getMazedists().getAllDistanceValues(), mazeConfig.getStartingPosition()[0], mazeConfig.getStartingPosition()[1]);
        		*/
    ...
    public void start(MazePanel panel) {
        ...
        Log.d("StatePlaying", "Was able to register currentdirction");
    ...
    public boolean keyDown(UserInput key, int value) {
        ...
        switch (key) {
            ...
            case Up: // move forward
                ...
                if (isOutside(px, py)) {
                    if (controlManually != null)
                        controlManually.moveToWin();
                    if (controlAnimation != null)
                        controlAnimation.moveToWin();
                }

            /*
            int[] tracker = getCurrentPosition();
            System.out.println(tracker[0]);
            System.out.println(tracker[1]);
            */
                break;
            case Left: // turn left
                rotate(1);
    ...
    private Handler slowRedraw = new Handler();
    ...
    private void slowedDownRedraw() {
        ...
        //slowRedraw.removeCallbacks(mUpdateTimeTask);
        //slowRedraw.postDelayed(mUpdateTimeTask,12);
    ...
    private Runnable mUpdateTimeTask = new Runnable() {
        public void run() {
            slowRedraw.postDelayed(this, 12);
        }
    };
    ...
    synchronized private void walk(int dir) {
        ...
        if (controlManually != null) {
            Log.v("StatePlaying","Took a step");
            controlManually.pathLength++;
        }
        draw();
