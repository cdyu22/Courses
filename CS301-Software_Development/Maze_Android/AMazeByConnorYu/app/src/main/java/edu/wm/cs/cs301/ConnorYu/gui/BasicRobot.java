...
import edu.wm.cs.cs301.ConnorYu.generation.MazeContainer;
...
public class BasicRobot implements Robot {
	Floorplan layout;
	PlayAnimationActivity mazeConnection;
	...
	public void setMaze(PlayAnimationActivity controller) {
		...
		layout = controller.maze.getFloorplan();
		mazeConnection = controller;
		Log.v("BasicRobot","Received command to set maze");
	}
	...
	public void setWinOrLoss(boolean win) {
		Log.v("BasicRobot","Received command to setup finishing variables");
	...
	public int[] getSpot(Direction direction) {
		...
		if(mazeConnection.statePlay.mazeConfig.isValidPosition(positionTracker[0], positionTracker[1])) {
		 ...
		 */
		if(!mazeConnection.maze.isValidPosition(positionTracker[0], positionTracker[1])) {
			int[] exit = new int[2];
			exit[0] = -10;
			exit[1] = -10;
			return exit;
	...
	public int[] getCurrentPosition() throws Exception {
		...
		currentSpace = mazeConnection.statePlay.getCurrentPosition();

		//If it's out of bounds either in the x or y position, it's complete, so set completed to true.
		if (currentSpace[0] < 0 || mazeConnection.maze.getWidth() <= currentSpace[0] ||
				currentSpace[1] < 0 || mazeConnection.maze.getHeight() <= currentSpace[1]) {
			completed = true;
			throw new Exception();
		}
	...
	public CardinalDirection getCurrentDirection() {
		...
		currentDirection = mazeConnection.statePlay.getCurrentDirection();
	...
	public boolean canSeeThroughTheExitIntoEternity(Direction direction) throws UnsupportedOperationException {
		...
		while (mazeConnection.maze.isValidPosition(positionTracker[0], positionTracker[1]) &&
				layout.hasNoWall(positionTracker[0], positionTracker[1], look)) {
			Log.v("BasicRobot",
					"X Variable: " + positionTracker[0] + "," +
							" Y Variable: " + positionTracker[1]);
	...
	public void rotate(Turn turn) {
		if (turn == Turn.LEFT) {
			...
			mazeConnection.statePlay.keyDown(UserInput.Left, 0);
		}
		else if (turn == Turn.RIGHT) {
			if (battery < getEnergyForFullRotation()/4)
				return;
			battery = battery - (getEnergyForFullRotation()/4);//BATTERY
			mazeConnection.statePlay.keyDown(UserInput.Right, 0);
		}
	...
	public void move(int distance, boolean manual) {
		while (0 < distance) {
			...
			mazeConnection.statePlay.keyDown(UserInput.Up, 0);
			odometer++;
			distance--;
		}
	...
	public void jump() throws Exception {
		try{
			...
			mazeConnection.statePlay.keyDown(UserInput.Jump, 0);
			odometer++;
			battery = battery - (getEnergyForStepForward());//BATTERY
			getCurrentPosition();
		} catch(Exception e){
