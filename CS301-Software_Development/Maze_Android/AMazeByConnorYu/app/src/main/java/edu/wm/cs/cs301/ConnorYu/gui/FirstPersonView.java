...
import edu.wm.cs.cs301.ConnorYu.generation.BSPBranch;
import edu.wm.cs.cs301.ConnorYu.generation.BSPLeaf;
import edu.wm.cs.cs301.ConnorYu.generation.BSPNode;
import edu.wm.cs.cs301.ConnorYu.generation.Floorplan;
import edu.wm.cs.cs301.ConnorYu.generation.Wall;
import java.io.File;
...
public class FirstPersonView {
	...
	private void drawBackground(MazePanel graphics, float percentToExit) {
		...
		// dynamic color setting: viewWidth, viewHeight, viewWidth, viewHeight/2
		// grey rectangle in lower half of screen
		//graphics.setColor(Color.DKGRAY);
		// dynamic color setting:
		graphics.setColor(MazePanel.getColorEncoding(0,240,0));
		graphics.addFilledRectangle(0, viewHeight, viewWidth, viewHeight);

		graphics.setColor(getBackgroundColor(percentToExit, false));
		graphics.addFilledRectangle(0, viewHeight, viewWidth, viewHeight/2);
	...
	private int getBackgroundColor(float percentToExit, boolean top) {
		 ...
		 */
		return MazePanel.getColorEncoding(104,218,255);
	}
	...
	private int blend(int c0, int c1, double weight0) {
		...
		if (weight0 > 0.95)
			return c0;
	    double r = weight0 * c0 + (1-weight0) * c1;
	    double g = weight0 * c0 + (1-weight0) * c1;
	    double b = weight0 * c0 + (1-weight0) * c1;

	    return MazePanel.getColorEncoding((int) r, (int) g, (int) b);
	  }
