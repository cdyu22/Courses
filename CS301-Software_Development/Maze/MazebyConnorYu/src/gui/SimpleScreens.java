...
public class SimpleScreens {
     ...
     * @param panel holds the graphics for the off-screen image
     */
	void redrawFinish(MazePanel panel, boolean winner, int battery, int pathLength) {
		Graphics g = panel.getBufferGraphics() ;
        if (null == g) {
            System.out.println(errorMsg) ;
        }
        else {
            redrawFinish(g, winner, battery, pathLength);
        }
	}
	/**
	 ...
	 */
	private void redrawFinish(Graphics gc, boolean winner, int battery, int pathLength) {
		...
		if (winner) {
			...
			centerString(gc, "Robot Battery Remaining: " + battery, 220);
			centerString(gc, "Robot Path Length: " + pathLength, 240);
			// write the instructions
			gc.setColor(blackWM);
			centerString(gc, "Hit any key to restart", 300);
		}
		if(!winner) {
			centerString(gc, "Sorry, you lose!", 100);
			// write some extra blurb
			updateFontAndColor(gc, smallBannerFont, greenWM);
