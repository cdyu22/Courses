...
public class Controller {
    ...
    //Variables the robot passes to controller, StateWinning will pull these from the controller to display at the ending screen
    boolean winner = true;
    int battery;
    int pathLength;
    
    //A boolean that's set to true if there is a driver that's trying to get to the exit
    //Lets SimpleKeyListener know that it should ignore all user inputs unless it's to return to title.
    ...
    public void switchFromGeneratingToPlaying(Maze config) {
       ...
       //Checks to see if driver and robot have been assigned.
        if (driver != null && robot != null) {
        	//Sets running to true which the SimpleKeyListener will use to make sure most keyInputs aren't registered
        	running = true;
        	
        	driver.setDistance(config.getMazedists());
        	
        	//Sets dimensions and gives the robot a connection to controller
        	driver.setDimensions(getMazeConfiguration().getWidth(), getMazeConfiguration().getHeight());
        	robot.setMaze(this);
        	
        	//Then tells the driver to drive to the exit
        	try {
        		driver.drive2Exit();
        	} catch (Exception e) {
        		e.printStackTrace();
        	...
        	//At the end, sets running to false so we let SimpleKeyListener listen to user inputs.
        	running = false;
        }
     ...
     * 
     * In addition to that: it also checks to see if the driver is running and
     * if SimpleKeyListener let the return to title command through.
     */
    public boolean keyDown(UserInput key, int value) {
    	/*
    	 * If the driver is running and isn't null, and we signal to SimpleKeyListener that 
    	 * we want to escape to title, we set running back to false and call stop on the robot,
    	 * which the driver is checking, terminating drive2exit when it notices the variable's change.   
    	 */
    	if (key == UserInput.ReturnToTitle && driver != null && running) {
    		running = false;
    		((BasicRobot) robot).stop();
    	}
