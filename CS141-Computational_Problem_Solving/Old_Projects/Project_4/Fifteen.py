...
CELL_COUNT = 4
CELL_SIZE = 60
CELL_SIZE_HALFX = (CELL_SIZE/2)-3
CELL_SIZE_HALFY = (CELL_SIZE/2)+3
GRID_ORIGINX = 130
GRID_ORIGINY = 100
            ...
            for col in range(CELL_COUNT):
                ...
                if self.__board[row][col] == -1:
                    ...
                    if self.__winner == True:
                        qp.setBrush(Qt.white)
                    qp.drawRect(GRID_ORIGINX + CELL_SIZE * col,GRID_ORIGINY + CELL_SIZE * row, CELL_SIZE, CELL_SIZE)
                    qp.drawText(GRID_ORIGINX + (CELL_SIZE * col) + CELL_SIZE_HALFX,GRID_ORIGINY + (CELL_SIZE * row) + CELL_SIZE_HALFY,str(self.__board[row][col]))
        qp.end()
        ...
        if 0 <= row <= 3 and 0 <= col <= 3:
            if self.__board[row][col] == -1:
                ...
                if row == 3:
                    ...
                    self.update1()
                self.__hint1 = False
                self.__moves += abs(col-self.__c1)
            elif self.__c1 == col:
                if col == 0:
                ...
                if col == 3:
                    ...
                    self.update2()
                self.__hint1 = False
                self.__moves += abs(row-self.__r1)
        self.update()
