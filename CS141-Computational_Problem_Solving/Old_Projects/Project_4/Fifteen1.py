...
#need to calculate inversions to test if the puzzle is solvable or not. Need to use the theorem to make sure the configuration is solvable.
#Generate grid, but before that make sure it's solvable, if not, then regenerate, until there's a configuration that's solvable.
#Should ignore clicks outside the grid. Ignore clicks that aren't valid,such as squares diagonal to the empty space, or when the game is over.
#If click any adjacent tile, it switches. Don't have to show sliding animation.
#Any squares clicked with th same row or column will slide all of them towards the empty space.
#Program needs a moves counter.
#Need write-up.
...
class Fifteen(QWidget): #-1 is the initial value to represent an empty square
    def __init__(self):
        super().__init__()
