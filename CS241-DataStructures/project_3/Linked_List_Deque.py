...
class Linked_List_Deque(Deque):
  ...
  def push_front(self, val):
      if len(self.__list) == 0:
          self.__list.append_element(val) #We cannot insert into an empty list.
      else:
          self.__list.insert_element_at(val,0)

  def pop_front(self):
      if len(self.__list) == 0:
          return # We do this for future methods too to avoid raising errors.
  ...
  def pop_back(self):
      ...
      #This and peek_back occur in constant time, as the linked list
      #starts walking from the back.
