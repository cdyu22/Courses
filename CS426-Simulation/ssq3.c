...
// #define DEMANDWAGE   12.0 
// #define SERVICETIME   (2.0/3.0)

#define DEMANDWAGE    6.0
#define SERVICETIME   0.5
  ...
  int main(void)
{
  ...
  while ((t.arrival < STOP) || (number > 0)) { //Will serve node past close
    ...
    if (t.current == t.arrival)  {               /* process an arrival, next event is arrival */
      totalArrival++;
      if (number < 3){
        number++;
        if (number == 1)
      ...
      if (t.arrival > STOP)  {
        t.last      = t.current;  // Only for statistical purposes
        t.arrival   = INFINITY;   // Next arrival will NEVER occur, always greater than STOP
      }
    }

    //OR
    else {                                        /* process a completion, next event is completion */
      index++;
      number--;
      profit += SNOCONEP;
      if (number > 0)
  ...
  printf("   total profit............. = %11.2d\n",    profit);
  printf("   total wages.............. = %11.2f\n",    wages);
  printf("   total revenue............ = %11.2f\n\n", (double) profit - wages);
  printf("   total arrivals........... = %11.2f\n\n", totalArrival);
